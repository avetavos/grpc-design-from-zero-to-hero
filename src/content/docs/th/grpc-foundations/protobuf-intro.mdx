...
export const encodeDiagram = `flowchart LR
  subgraph json["JSON บน wire"]
    j["{#quot;id#quot;:1,#quot;name#quot;:#quot;Ada#quot;,#quot;email#quot;:#quot;a@x.io#quot;}<br/>— key ซ้ำเป็น text"]
  end
  subgraph pb["Protobuf บน wire"]
    p["[1→1][2→Ada][3→a@x.io]<br/>— tag ที่เป็นหมายเลข + ค่า binary"]
  end`;
...
Protocol Buffers เป็นทั้งภาษา schema และ binary wire format ที่กระชับ ทริกสำคัญคือ **field number**: wire พก tag ที่เป็นหมายเลข ไม่ใช่ชื่อ — ซึ่งทำให้ message เล็ก, decode เร็ว และ rename ปลอดภัย schema แบบ contract-first นี้คือสิ่งที่ทุก feature อื่นของ gRPC สร้างต่อขึ้นมา
</Callout>
...
export const quizProto = [
  ...
  },
  {
    ...
    options: [
      'ช้ากว่าตอน parse',
      'คนอ่านไม่ออกบน wire ถ้าไม่มี `.proto`',
      'แทนข้อมูลแบบซ้อนกันไม่ได้',
      'ใช้ได้แค่ใน Go',
    ],
    answer: 1,
  },
