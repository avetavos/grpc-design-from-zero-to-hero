...
call ของ gRPC จบด้วย **status** — `code`, `message`, และ `details` (optional) — ไม่ใช่ HTTP code เลือก code ให้แม่นยำ (`NOT_FOUND` vs `INVALID_ARGUMENT` vs `FAILED_PRECONDITION`; `UNAUTHENTICATED` vs `PERMISSION_DENIED`) เพราะ client retry และแตกเงื่อนไขจาก code เหล่านี้ หยิบ `google.rpc.Status` details มาใช้เมื่อ client ต้องการรายละเอียดแบบ machine-readable
</Callout>
